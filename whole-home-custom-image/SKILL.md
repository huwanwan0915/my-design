...
1. 先看 Codex 全局配置是否已经提供了图片专用 key、图片模型和兼容接口地址
   - `~/.codex/auth.json`
   - `~/.codex/config.toml`
2. 如果 `config.toml` 里配置了 provider `base_url`，图片请求默认优先走该兼容接口，不要硬编码回 `api.openai.com`
3. 只有在本地脚本已经正确读取了 `OPENAI_IMAGE_API_KEY`、`OPENAI_IMAGE_MODEL`、`base_url` 后，才允许走本地脚本出图
4. 如果会话内已有稳定的直接出图路径，优先使用同一条路径连续完成同类任务，不要中途切换到另一条未验证链路
...
5. 再定义允许变化的内容：
   ...
   - 灯带、主灯、氛围光
   - 软装和摆件
6. 输出时默认给 1 个主版本 + 2 个轻微变化版本，除非用户明确只要单版。
