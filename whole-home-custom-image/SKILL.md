...
当用户要求还原某个木纹、板材花色、木饰面或反馈“木纹不真实 / 不高清 / 和实拍差别大”时，按下面流程执行，不要只靠文字描述生成。

1. 必须优先收集实拍参考图：
   - 尽量让用户提供 1–3 张真实木纹照片或供应商板样截图
   - 最好同时覆盖近景纹理、整体板面和实际光照环境
   - 如果有目标色号、板材型号、品牌样板、花色编码，要一起写入提示词
2. 使用“参考图驱动”而不是纯文字生成：
   - 把参考图作为材质基准输入给图像模型
   - 明确要求生成结果贴近参考图的木色、纹理方向、颗粒/孔隙密度、哑光/亮光程度
   - 不要只写 `walnut`、`oak`、`wood grain` 这类泛化词，否则模型容易生成通用木纹
3. 先做小样或局部校色：
   - 优先先生成局部木纹样板、局部柜门或小范围材质替换
   - 等用户确认颜色、纹理密度、质感后，再扩展到完整空间图或产品图
4. 明确材质类型和工艺边界：
   - 在提示词中区分浅橡木、胡桃木、樱桃木、白蜡木、烟熏木、科技木皮、实木多层板、免漆板等
   - 不同木材和不同板材工艺的纹理规律不同，不要互相套用
   - 对定制板材饰面，要强调“板材饰面 / 木纹装饰面”的真实尺度，不要过度生成天然原木的大结疤、大山纹
5. 生成后做二次增强和偏差修正：
   - 提高清晰度和近看纹理细节
   - 补充真实木孔、细小纤维、自然色差和微弱层次
   - 降低 AI 塑料感、糊纹、重复贴图感和过强规律性
   - 每轮只收敛 1–2 个关键偏差，例如只压暗底色、只增加细纹、只降低红黄偏色

如果用户强调“只改木纹部分，其他布局和颜色不变”，必须使用局部编辑/遮罩思路：只开放木纹区域编辑，锁定白色柜门、墙面、地面、灯光、家电、软装、构图和整体色调。提示词里要反复写明 `edit only the masked wood areas; keep everything else unchanged` 或同等约束。
